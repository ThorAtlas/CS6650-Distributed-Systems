...
public class RedBlackBST<Key extends Comparable<Key>, Value> {
  ...
  //A private method to swap children around to keep the tree balanced/left leaning
  private void swapKids(Node node){
    if(node == null){
      return;
    }
  ...
  private boolean isRed(Node x) {
    ...
    //root is always black so can't be red
    if(x == root && size(root) ==1){
      return false;
    }

    //Nodes without siblings are red because newly added nodes are red. So we check if it has
    //a sibling
    Node currentNode = root;
    while(currentNode != null) {
      if (currentNode.left == x && currentNode.right == null) {
        return true;
      }
      if (currentNode.right == x && currentNode.left == null) {
        return true;
      }
      //The node has a sibling, so now we got to check if its siblings are red
      if (currentNode.left == x || currentNode.right == x) {
        break;
      }
      //here we continue the search

      //check if either child is null
      if (currentNode.left == null || currentNode.right == null) {

        //if one is null then we move on to the other
      ...
      //if there isn't just one null node then they are both full fledged nodes
      else {

        //vlaue to see if x is bigger or smaller than current node
        int compare = x.key.compareTo(currentNode.key);

        //check if left node of the current is bigger than right node. If it is not there is an
        //error somewhere.
        if (!(currentNode.left.key.compareTo(currentNode.right.key) > 0)) {
          //if x is smaller than the current node we continue downwards to the left towards bigger
          //nodes
          if (compare < 0) {
            currentNode = currentNode.left;
          }
          //if x is bigger than the current node we continue downwards to the right towards smaller
          //nodes
          else if (compare > 0) {
            currentNode = currentNode.right;
          } else {
